var mostrarArOn = false;

var mostrarArOff = false;

var mostrarPainelDeArCondicionado = false;
var contadorIntervaloArCondicionado = 0;

var arCondicionadoDesligado = false;

var opcaoDeArCondicionado;

function arOpcaoOn(){

    if(mostrarArOn === false){
        return
    }

    contadorIntervaloArCondicionado = 0;
    mostrarArOff = true;
    ...
    opcaoDeArCondicionado= "luzOn"
}

 function arOpcaoOff(){

    if(mostrarArOff === false){
        return
    }
    ...
    opcaoDeArCondicionado = "luzOff"
 } 

 function painelDeArCondicionado(){
     if(mostrarPainelDeArCondicionado === false){
         return
     }
         ...
         var intervaloArCondicionado= setInterval(()=>{
            if(contadorIntervaloArCondicionado === 10){
                if(arCondicionadoDesligado === false){
                    mostrarArOn = false;
                    mostrarArOff = false;
                    clearInterval(intervaloArCondicionado);
                    telaInicial(dinoBebe, frameBebeMovimento1);    
                    return 
                }
                if(arCondicionadoDesligado === true){
                    mostrarArOn = false;
                    mostrarArOff = false;
                    clearInterval(intervaloArCondicionado);
                    telaInicialApagada();  
            ...
            contadorIntervaloArCondicionado ++;
            console.log("intervalo luz: " + contadorIntervaloArCondicionado);
         }, 1000)
     }
 }


$(".selecao-esquerda").click(arOpcaoOn);
$(".selecao-direita").click(arOpcaoOff);
